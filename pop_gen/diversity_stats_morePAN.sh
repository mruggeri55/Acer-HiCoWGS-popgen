...
########### PI ##############
# 9 Oct 2025
# calculating pi -- for populations
# change config to calculate at different levels
#!/bin/bash
#SBATCH --partition=epyc-64  
#SBATCH --ntasks=1
#SBATCH --mem=32G
#SBATCH --time=03:00:00
#SBATCH --error=/scratch1/mruggeri/AcerHapPanel/pop_gen/pi/2025/%x_%j.err
#SBATCH --output=/scratch1/mruggeri/AcerHapPanel/pop_gen/pi/2025/%x_%j.out
#SBATCH --mail-type=END
#SBATCH --mail-user=mruggeri@usc.edu
#SBATCH --array=1-5
# enter your job specific code below this line 
...
config=/scratch1/mruggeri/AcerHapPanel/pop_gen/lists/config_region.txt
export POP=$(awk -v ArrayTaskID=$SLURM_ARRAY_TASK_ID '$1==ArrayTaskID {print $2}' $config)
OUT=/scratch1/mruggeri/AcerHapPanel/pop_gen/pi/2025/morePAN/
VCF=/scratch1/mruggeri/AcerHapPanel/vcfSanger/final_merge/morePAN/snps.5bpind.merged.morePAN.v2.vcf.gz # note this includes both monomorphic and polymorphic loci
lists=/scratch1/mruggeri/AcerHapPanel/pop_gen/lists
# calculate pi in 10kb windows
vcftools --gzvcf $VCF --max-missing 0.9 --min-meanDP 10 --max-meanDP 50 --keep $lists/$POP'.pop.list' \
 --window-pi 10000 --window-pi-step 2000 --out $OUT/$POP 
# then concatenate all together once this is done -- the following line will concatenate output files and add a column for filename
for f in `ls *.windowed.pi`; do
	awk -v filename="$(basename "$f")" 'FNR > 1 {print $0 "\t" filename}' "$f" >> pi_by_pop.txt;
	done
# added header using nano then sftped to computer to visualize in R

# trying also with larger window size -- 100kb
# adjusted window size and added "100kb.windowed.pi" as suffix
for f in `ls *.100kb.windowed.pi.windowed.pi`; do
	awk -v filename="$(basename "$f")" 'FNR > 1 {print $0 "\t" filename}' "$f" >> pi_by_pop_100kb.txt;
	done

# PAN looks like lower pi than others, double checked and am using correct files so there shouldn't be any clones


####### TAJ D #######
# 9 Oct 2025
# running tajimas d as job -- TajD.sh
# make list of pop names
awk '{print $2}' $lists/config_region.txt | grep -v POP > $lists/pop.list
#!/bin/bash
#SBATCH --partition=epyc-64  
#SBATCH --ntasks=1
#SBATCH --mem=32G
#SBATCH --time=04:00:00
#SBATCH --error=/scratch1/mruggeri/AcerHapPanel/pop_gen/TajD/morePAN/%x_%j.err
#SBATCH --output=/scratch1/mruggeri/AcerHapPanel/pop_gen/TajD/morePAN/%x_%j.out
#SBATCH --mail-type=END
#SBATCH --mail-user=mruggeri@usc.edu
module load vcftools
OUT=/scratch1/mruggeri/AcerHapPanel/pop_gen/TajD/morePAN
VCF=/scratch1/mruggeri/AcerHapPanel/vcfSanger/final_merge/morePAN/snps.5bpind.merged.morePAN.v2.vcf.gz # note this includes both monomorphic and polymorphic loci
lists=/scratch1/mruggeri/AcerHapPanel/pop_gen/lists
# calculate Tajimas D in 100kb windows
for POP in `cat $lists/pop.list`; do
vcftools --gzvcf $VCF --max-missing 0.9 --min-meanDP 10 --max-meanDP 50 --keep $lists/$POP'.pop.list' \
 --TajimaD 100000 --out $OUT/$POP;
 done

# then concatenate all together once this is done -- the following line will concatenate output files and add a column for filename
for f in `ls *.Tajima.D`; do
	awk -v filename="$(basename "$f")" 'FNR > 1 {print $0 "\t" filename}' "$f" >> TajD_by_pop.txt;
	done

# note some windows have zero or very few snps -- need to filter these


###### PIXY ######## -- the following code is adapted from Carly's code
# Oct 22, 2025
...
##### trying ADZE for allelic richness
# 25 Apr 2026
# downloaded ADZE from https://rosenberglab.stanford.edu/adzeDownload.html to shared software folder in working directory
# PATH = /project2/ckenkel_26/software/ADZE-1.0
